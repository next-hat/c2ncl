...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Hash, Default)]
#[serde(untagged)]
pub enum EnvTypes {
    String(String),
...
impl fmt::Display for EnvTypes {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let env_type = match self {
            EnvTypes::Bool(v) => v.to_string(),
            EnvTypes::String(v) => v.to_string(),
            EnvTypes::Number(v) => v.to_string(),
            EnvTypes::Null => "".into(),
        };
        write!(f, "{env_type}")
    }
